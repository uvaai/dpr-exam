...
To be able to take better care of the animals, each species will go to a specific shelter location. Implement the function `separate_species(shelter, shelter_locations)` that takes the `shelter` dictionary and a dictionary of shelter locations for each species with the format `{species: location}`, and returns a new dictionary with the format `{location: {species: {name: {age, adopted}}}}`.
...
Test the function with:
    ...
    shelters = separate_species(shelter, shelter_locations)
    print(shelters)
